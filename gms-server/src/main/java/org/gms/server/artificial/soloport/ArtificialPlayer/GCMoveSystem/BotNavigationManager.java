...
final class BotNavigationManager {
    ...
    // portal execution 鈥?movement, attacks and every other action continue unaffected.
    private static final long PORTAL_USE_COOLDOWN_MS = 250L;
    // Intra-map portal shortcuts only fire once the bot is LANDED (never midair 鈥?e.g. down-jumping
    ...
    // it's in range. Positional jitter like a launch window 鈥?extra walk ticks, NOT a standing wait.
    private static final int PORTAL_ENTER_EXTRA_TICKS_MAX = 3;
    // Terminal warns are for the absolute worst searches only 鈥?the perf monitor already
    ...
    static NavigationDirective resolveTarget(BotMovementState entry, Point rawTargetPos, boolean runAiTick) {
        ...
        try {
            ...
            if (bot.getMap().isSwim()) {
                ...
                // ledge-drops, ropes, and ground jumps. Engage the heuristic fallback 鈥?                // it walks off ledges into water, picks up nearby ropes, and jumps onto
            ...
            if (entry.navGraph != graph) {
                ...
                // fallback won): committed edges were calibrated against the old instance 鈥?                // their windows and launch steps don't transfer. Drop and replan right now.
            ...
            // moving is parked, not approaching 鈥?e.g. a window recorded a few px away from
            ...
            // A TrainingBot never needs a cross-platform rope route to grind.  The
            // donor's generic navigation intentionally uses CLIMB edges, but on a
            // v83 observer this can overlap an attack packet and leave the fake
            // character visually stuck on a ladder.  Snap to the selected ground
            // target instead: training remains local, deterministic and rope-free.
            if (entry.grinding && edge.type == BotNavigationGraph.EdgeType.CLIMB && rawTargetPos != null) {
                Point ground = BotPhysicsEngine.findGroundPoint(bot.getMap(), rawTargetPos);
                BotPhysicsEngine.teleportTo(entry, bot, ground != null ? ground : rawTargetPos);
                BotMovementManager.resetEntryStateAfterTeleport(entry);
                BotMovementManager.broadcastMovement(entry);
                clearNavigation(entry);
                entry.lastNavDecision = "grind-no-climb";
    ...
    private static void notifyWarmup(BotMovementState entry, Character bot) {
        ...
        // Only count walkable footholds when we are about to send 鈥?lazy, inside throttle gate
    ...
    static BotNavigationGraph.Edge reuseCommittedEdge(BotNavigationGraph graph,
        ...
        if (startRegionId == edge.toRegionId && !entry.inAir && !entry.climbing
            ...
            // bot), not by a region change 鈥?don't retire on region match.
            return null;
        }
        // Once the resolved target is back in the bot's current region, a committed edge that
        // would leave that region is *usually* stale 鈥?follow/formation loops where the bot keeps
        ...
        // (the snap-back case) 鈥?then the edge was planned for a different target and is truly stale.
        ...
        // While climbing, always keep the edge 鈥?findGroundFoothold gives false positives
        ...
        // this arc (destination or unmapped) 鈥?prevents looping in a wrong region mid-air.
    ...
    private static NavigationDirective tryExecuteJump(BotNavigationGraph graph,
        ...
        if (!canExecuteSelectedJumpFromCurrentPosition(graph, entry, bot.getMap(), botPos, edge)) {
            ...
            // Grab the rope and descend 鈥?tickClimbing will naturally drive toward edge.startPoint.
            if (edge.startPoint.y > botPos.y) {
                ...
                if (fromRegion != null && fromRegion.isRopeRegion) {
                    ...
                    if (rope != null && canGrabRopeAtCurrentPosition(botPos, rope)) {
                        // Attach at bot's current Y 鈥?tickClimbing will drive it down to startPoint.
        ...
        if (deepenJumpLaunchOneStep(graph, entry, bot.getMap(), edge)) {
            ...
            return null; // steering follows the bumped launch X 鈥?walk a step deeper first
        }
        ...
        // Directional jumps never wait: the launch snaps to 卤walkSpeed regardless of slide.
     ...
     * approach). A borderline arc 鈥?e.g. real speed sitting between graph buckets 鈥?that
    ...
    private static boolean deepenJumpLaunchOneStep(BotNavigationGraph graph,
        ...
        if (!edge.containsLaunchX(deeperX)
            ...
            entry.navJumpLaunchDelaySteps = 0; // no legal room deeper 鈥?fire from here
            return false;
        }
    ...
    private static NavigationDirective tryExecuteClimbEntry(BotNavigationGraph graph,
        ...
        if (canGrabRopeAtCurrentPosition(botPos, rope)) {
            // Bot is already within the rope's Y range 鈥?attach at its current Y, not the edge
    ...
    private static NavigationDirective tryExecuteClimbExit(BotNavigationGraph graph,
        ...
        if (edge.launchStepX == 0) {
            ...
            // topY. Nav just lets the bot climb 鈥?the edge completes when the bot transitions
    ...
    private static NavigationDirective tryExecutePortalEdge(BotMovementState entry,
        ...
        // the portal's trigger box mid-fall must NOT warp 鈥?the bot lands and walks in first.
     ...
     * portal 鈥?see BotNavigationProbe --measure). The legacy .heuristic and the
        ...
        try {
            ...
            while (!open.isEmpty()) {
                ...
                for (BotNavigationGraph.Edge edge : graph.getOutgoing(current.state.regionId)) {
                    ...
                    // only when the bot enters a portal *through the exit* of the one it just took 鈥?                    // i.e. it landed on a portal and immediately re-enters without walking. A
    ...
    private static boolean usePortal(Character bot, int portalId) {
        ...
        // Bots are clientless (shared BotClient) 鈥?warp the bot Character directly, never
    ...
    // ponytail: route-diversification knobs 鈥?calibrated on map 10000 via BotRouteDiversityTest.
    ...
    private static boolean isTopRopeJumpExitReady(Rope rope, Point botPos, BotNavigationGraph.Edge edge) {
        ...
        // are precomputed by simulateRopeJumpLanding 鈥?launching from any other Y misses the
    ...
    static int resolveTargetRegionId(BotNavigationGraph graph,
        ...
        if (entry.following
            ...
            // exact equality check below would miss 鈥?explicitly look for a rope

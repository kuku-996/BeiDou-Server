...
// ROAM: anchor-free grind for un-campable maps (no ledge holds a harvestable pack 鈥?tiny platforms,
// jumpy mobs). Each beat: loot underfoot, seek the nearest live mob across ledges and close on it 鈥?// walking / arced-jumping / blinking 鈥?swing when in range, repeat. Reuses the same shared engage
...
final class RoamStrategy implements GrindStrategy {
    ...
    // it stands on a ledge in the SAME detected stack as the bot 鈥?a validated blink/hop pair (slime
    ...
    // (Audit III 2.3 鈥?roam maps previously had no crowding signal at all).
    ...
    public void tick(Character chr) {
        ...
        if (t == null) {
            ...
            if (!b.loot.tryWalkAndLoot(chr, leash[0], leash[1], ROAM_LOOT_SEARCH_PX)) {
                GCMovement.stop(chr); // no mob in seek range this beat 鈥?hold; re-scan next tick
                b.engaged = false;
            }
    ...
    private void approachRoam(Character chr, Monster mob) {
        ...
        if (now() < b.attackWalkLockUntil) {
            return; // just swung 鈥?hold, then move
        }

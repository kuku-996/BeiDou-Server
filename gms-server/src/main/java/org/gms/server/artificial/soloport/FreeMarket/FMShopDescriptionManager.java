...
//            debugprint("compatWithfirst: " + compatibleWithFirst);
    ...
    public void generateDescription(HiredMerchantArtificial merchant) {
        HiredMerchantArtificial.shopTypes primary = merchant.getPrimary();
        String category = switch (primary == null ? HiredMerchantArtificial.shopTypes.Common : primary) {
            case Thief, Stars -> "飞侠用品";
            case Warrior -> "战士装备";
            case Mage -> "法师装备";
            case Bowman -> "弓手装备";
            case Chair -> "椅子收藏";
            case Scroll, DarkScroll, Mastery -> "卷轴专柜";
            case Potion -> "冒险补给";
            case ETC -> "杂货商店";
            default -> "精品商店";
        };
        List<String> suffixes = List.of("欢迎选购", "价格可议", "诚意出售", "好物不断", "先到先得");
        String suffix = suffixes.get(random.nextInt(suffixes.size()));
        merchant.setDescription(category + "・" + suffix);

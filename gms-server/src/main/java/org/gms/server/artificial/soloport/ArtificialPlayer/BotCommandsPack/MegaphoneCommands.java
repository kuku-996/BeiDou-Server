...
public class MegaphoneCommands {
    ...
    public static void BotItemMegaphone(Character fakechar, String message, Item item) {
        String msg = fakechar.getName() + " : " + SoloMaplingChineseLocalization.botSpeech(message, PhraseCategory.MEGAPHONE); // medal + // optional medal
    ...
    private static void BotSuperMegaphone(Character fakechar, String msg, boolean ear) {
        String localized = SoloMaplingChineseLocalization.botSpeech(msg, PhraseCategory.MEGAPHONE);
        Server.getInstance().broadcastMessage(world.getId(), PacketCreator.serverNotice(3, channel.getId(), fakechar.getName() + " : " + localized, ear));
    }
    ...
    private static void BotAvatarMegaphone(Character fakechar, String message, int itemId) {
        LinkedList list = stringTo4LineLinkedList(SoloMaplingChineseLocalization.botSpeech(message, PhraseCategory.MEGAPHONE));
        String medal = "";
        boolean ear = true;
        Server.getInstance().broadcastMessage(world.getId(), PacketCreator.getAvatarMega(fakechar, medal, channel.getId(), itemId, list, ear));
//      //  TimerManager.getInstance().schedule(() -> org.gms.server.getInstance().broadcastMessage(world.getId(), PacketCreator.byeAvatarMega()), SECONDS.toMillis(10));
    }
    ...
    private static void BotMapleTVFull(Character fakechar, String msg, int itemId, Character partner) {
        ...
        LinkedList messages = stringTo5LineLinkedList(SoloMaplingChineseLocalization.botSpeech(msg, PhraseCategory.MEGAPHONE));

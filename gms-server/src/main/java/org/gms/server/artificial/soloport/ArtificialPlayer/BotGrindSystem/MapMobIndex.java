...
public final class MapMobIndex {
    ...
    // A static spawn point: raw WZ position (x, cy) plus the ledge key of the foothold it sits on 鈥?    // derived from the WZ foothold tree's layer/group structure (-1 when the fh id is unknown). Not the
    ...
    // Raw WZ spawn points (x, cy, foothold-group ledge key). Static data straight off the map img 鈥?no
    // live map load, no nav bake 鈥?so DECIDE can estimate a map's claimable-spot count before any bot
    // ever grinds it.
            ...
            for (Data group : layer) {
                ...
                for (Data f : group) {
                    try {
                        ...
                        // non-numeric foothold node 鈥?skip
                    }
                }
            }
        }
    ...
    private static Data loadMapData(int mapId) {
        ...
        if (!link.isEmpty()) {
            try {
                ...
                // malformed link 鈥?use the map as-is
            }
        }
        return mapData;
    }

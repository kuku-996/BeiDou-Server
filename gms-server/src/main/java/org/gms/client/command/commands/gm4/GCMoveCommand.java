...
/*
 * !gcmove 鈥?GreenCat dynamic (calculation-based) movement test/control.
...
public class GCMoveCommand extends Command {
    ...
    public void execute(Client c, String[] params) {
        ...
        // 'ropecheck' inspects the GM's own map ropes against the top-exit probe 鈥?no bot.
        if (sub.equals("ropecheck")) {
            GCMovement.ropeCheckReport(player).forEach(player::dropMessage);
            return;
        }

        // 'lod' is the LOD measurement tooling (M0) 鈥?load/unload/stats, no specific bot.
        if (sub.equals("lod")) {
        ...
        switch (sub) {
            ...
            case "fj" -> {
                ...
                // fit) and report the measured ground travel 鈥?the numbers feed GCMovementSkills.FJ_TRAVEL_PX.
                float scale = 1f;
                if (params.length >= 3) {
                    try {
                        scale = Float.parseFloat(params[2]);
                    } catch (NumberFormatException e) {
                        player.dropMessage("Usage: !gcmove fj <botId> [scale 0.2-1.0] 鈥?bad scale, using 1.0");
                    }
                }
    ...
    // !gcmove lod [stats|load <n>|unload] 鈥?LOD measurement tooling (M0).

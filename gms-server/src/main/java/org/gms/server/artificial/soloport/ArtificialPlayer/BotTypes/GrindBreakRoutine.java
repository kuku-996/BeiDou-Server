...
import static org.gms.server.artificial.soloport.ArtificialPlayer.BotCommandsPack.SocialCommands.botSetChalkboard;
...
// the sit / stand / rope-hang / "brb" chalkboard / sleepy mutter, and the resume bookkeeping 鈥?extracted
...
final class GrindBreakRoutine {
    ...
    void schedule(long grindUntilMs, boolean resumingFromBreak) {
        ...
        if (sessionLen <= BREAK_EDGE_MS * 2) {
            return; // shortened (mid-session) first grind 鈥?not worth a break
        }
    ...
    // Start the break: resolve the rest spot BEFORE any side effect 鈥?unobserved with no nearby
    ...
    // One BREAK_TRAVEL tick: walk (or climb) to the rest spot on the same map. A no-progress watchdog 鈥?    // not a flat cap 鈥?lets a legit multi-rope trek to a safe upper ledge finish as long as it keeps
    ...
    boolean tickTravel(Character chr) {
        ...
        if (stalled || tooLong) {
            ...
            GCMovement.stop(chr); // already fell back to a local safe spot once 鈥?rest here
            return true;
        }
    ...
    boolean tickRest(Character chr) {
        if (!restStarted) {
            ...
            if (restKind == RestSpotFinder.Kind.GROUND) {
                ...
                // Don't plant a chair on it 鈥?stand instead (observed maps only; unobserved is unseen).

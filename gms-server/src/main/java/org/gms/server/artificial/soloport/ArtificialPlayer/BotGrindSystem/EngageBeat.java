...
final class EngageBeat {
    ...
    // 鈹€鈹€ Movement style (advanced classes blink/dash toward mobs; PLANTED default keeps low-levels organic) 鈹€鈹€
    ...
    private static final double JUMP_ATTACK_CHANCE = 0.68;   // thieves hop (arced) most engages 鈥?their signature vs planted warriors/archers
    ...
    // 鈹€鈹€ Ranged kiting (bowmen back off when a mob closes inside the comfort band) 鈹€鈹€
    ...
    // 鈹€鈹€ Attack tempo (real classes can't walk mid-attack; only jumps/teleports move while attacking) 鈹€鈹€
    ...
    // Returns true when the beat was spent moving/turning 鈥?the caller swings next tick instead.
    ...
    boolean skillMoveToward(Character chr, int tx, int ty) {
        ...
        // stacked target nearly overhead (small dx, big dy) 鈥?flash jump stays horizontal-only (its
    ...
    // CURRENT ledge 鈥?it doesn't see a drop mid-arc onto a mini-platform below (and an unbaked region gets
    ...
    // and answers "can't tell" as false, so an unbaked map falls through to the old dash behaviour 鈥?the
    ...
    // anchor watchdog then yanks it back from 鈥?the visible "backwards teleport" oscillation. Gating on farX
    ...
    // refusal would break STACK 鈥?stacked footholds are distinct nav regions so onDifferentLedge is ALWAYS true
    ...
    // closes in an arc instead of pogo-ing, but occasionally AWAY (the mini-kite 鈥?leap back and let the
    ...
    // mini-kite too. Only the AWAY hop needs this 鈥?a toward hop lands near an in-leash target.
    ...
    // AoE step-into-the-pack flourish: an AoE-capable class (not RANGED 鈥?those want distance) that's in

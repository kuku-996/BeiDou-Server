 ...
 * BotFallbackMovementManager) are static and mutate these fields in-place across calls 鈥? * the fields are the contract.
 *
...
class BotMovementState {
    ...
    // 鈹€鈹€ Physics integrator state (float "shadow" position the engine advances) 鈹€鈹€
    float velY = 0f;
    double hspeed = 0.0;
    double physX = 0.0;
    double physY = 0.0;
    ...
    // 鈹€鈹€ Swim intent (set by movement layer, consumed by physics) 鈹€鈹€
    ...
    boolean fixedAirArc = false;          // committed nav arc 鈥?disables air steering/drag
    // Flash Jump (Hermit/NL). execFlashJump launches a normal jump then arms pendingFlashJump; the
    // airborne integrator injects the (卤550,-350) dash impulse at the apex (velY>=0) and raises
    ...
    // smaller = reduced impulse fired part-way up the ascent 鈥?the short flash jump that fits a small
    ...
    // 鈹€鈹€ Mode flags read by nav/movement (inert under pure-movement scope) 鈹€鈹€
    ...
    // 鈹€鈹€ Foothold index, rebuilt on map change 鈹€鈹€
    int lastMapId = -1;
    Map<Integer, Foothold> fhIndex = new HashMap<>();

    // 鈹€鈹€ Spacing / stagger + tick gating 鈹€鈹€
    int followOffsetX = 0;
    ...
    // LOD coarse (M2): the slim analytic-movement record used while the bot is unobserved 鈥?it moves
    ...
    // 鈹€鈹€ Player reaction (ported pathAware): pause/chat at a nearby real player while roaming an observed
    ...
    // 鈹€鈹€ "Move here" / "farm here" targets (set by the GCMovement fa莽ade) 鈹€鈹€
    ...
    // 鈹€鈹€ Navigation state (written by NM, read by MM) 鈹€鈹€
    ...
    // 鈹€鈹€ Stuck detection & unstuck 鈹€鈹€
    int stuckMs = 0;
    ...
    // 鈹€鈹€ Movement-broadcast packet cache (dedup of no-op move packets) 鈹€鈹€

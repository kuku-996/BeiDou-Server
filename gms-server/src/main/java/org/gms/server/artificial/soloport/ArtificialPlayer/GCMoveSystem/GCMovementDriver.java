...
final class GCMovementDriver {
    ...
    // through a foothold gap (or off the side) free-falls forever 鈥?the frozen-air watchdog only fires once
    ...
    // Recompute a bot's movement profile on this cadence so runtime changes (chiefly party Haste 鈥?    // joining/leaving a party with a high-level thief) take effect without needing a map change.
     ...
     * cadence that matches the bot's tier 鈥?TICK_MS (50 ms) when its map is observed,
    ...
    private static void safeTick(BotMovementState entry) {
        try {
            org.gms.server.artificial.soloport.server.BotPerfStats.MOVEMENT_TICKS.increment();
            tick(entry);
        } catch (Throwable t) {
            // A thrown exception must not break the self-reschedule chain 鈥?swallow so the bot keeps ticking.
        }
    }
    ...
    private static void tick(BotMovementState entry) {
        ...
        // broadcast 鈥?never run nav / steering / player-reaction / contact-knockback below, any of which
        ...
        // LOD promotion (M2): the bot was moving analytically (coarse) and a player just arrived 鈥?        // rebuild the physics shadow at its current interpolated position so physics resumes cleanly
        ...
        // over the baked edge times 鈥?no physics, no broadcast 鈥?so it costs ~nothing. Falls through to
     ...
     * position so the resumed physics tick doesn't snap from stale shadow coordinates. Minimal 鈥?full
    ...
    // (a natural portal sits higher 鈫?keep its height), floats the "client load" beat, then drops.
    // Guarantees a visible spawn鈫抐loat鈫抎rop even when the arrival portal sits on the ground. Mirrors
    ...
    static void onMapChange(BotMovementState entry, Character bot) {
        ...
        // If a real player is already on this map, mark it observed NOW so the visible spawn (float 鈫?        // drop, jump stance) and FULL combat start this tick instead of waiting up to one observer poll.
        if (ObserverTracker.hasRealPlayerNow(map)) {
            ...
            org.gms.server.artificial.soloport.ArtificialPlayer.BotMapEntryResponder.onBotArrivedObserved(bot);
        }
        ...
        BotMovementManager.resetEntryStateAfterTeleport(entry);
        // Only play the visible spawn鈫抐loat鈫抎rop where a player can actually see it; on unobserved
        ...
        if (ground != null && ObserverTracker.isActiveMap(bot.getMapId())) {
            ...
            // so the bot floats at/above the portal a beat, then drops 鈥?every time, not just for
            ...
            // Unobserved (or no floor below): just stand where we landed 鈥?no float.
    ...
    // 鈹€鈹€ Stuck detection (faithful port; recovery teleport inlined) 鈹€鈹€
    ...
    private static void tickFallOffMapRecovery(BotMovementState entry) {
        ...
        if (vr == null || vr.width <= 0 || vr.height <= 0) {
            return; // no usable VR bounds 鈥?can't tell inside from outside
        }
        Point pos = bot.getPosition();
        if (pos == null) {
            return;
        ...
        BotMovementManager.resetEntryStateAfterTeleport(entry);
        // De-thrash: resetEntryStateAfterTeleport only clears NAV state, leaving moveTarget 鈥?so the bot

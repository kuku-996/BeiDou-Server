...
import static org.gms.server.artificial.soloport.ArtificialPlayer.BotLogic.isNpcPresent;
import static org.gms.server.artificial.soloport.ArtificialPlayer.BotTypes.OPQ.OPQConstants.CHAMBERLAIN_EAK;
 ...
 * (bots have no real client, so the OPQ NPC script does not warp them 鈥?the
 ...
 * tells a bot what state to be in 鈥?bots decide their own state from map + the
...
public class OPQOrchestrator {
    ...
    public synchronized Integer assignCloudReactor(OPQBot bot) {
        ...
        // Re-validate any existing assignment first 鈥?if dead, drop it and re-pick.
...
//                + ") -> " + bot.getChr().getName());
    ...
    /**
     * Hard stop 鈥?orchestrator idles, bots fall back to INACTIVE.
     */
     ...
     * <p>
     * Stage 1 completion: bots are not the trigger 鈥?they only know they've
    ...
    private void tick() {
        ...
        if (allBotsLostParty()) {
            opqLog("All registered bots lost their party 鈥?resetting run.");
            resetForNewRun();
            return;
        }
        ...
        switch (phase) {
            case STAGE_1 -> {
                if (!context.isStage1Complete() && isChamberlainSpawned()) {
                    opqLog("Stage 1 complete 鈥?Chamberlain Spawned");
            ...
            case STAGE_2 -> {
                if (!context.isStage2Complete() && allRegisteredBotsComplete()) {
                    opqLog("Stage 2 complete 鈥?all registered bots reported done.");
     ...
     * org.gms.server. Returns null if there's no leader yet, no bots registered, or

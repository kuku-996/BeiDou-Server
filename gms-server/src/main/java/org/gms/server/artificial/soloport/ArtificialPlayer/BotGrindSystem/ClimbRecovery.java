...
// now reads the brain's shared target (any live target, not the camp-validated sticky one 鈥?for a
...
final class ClimbRecovery {
    ...
    // Back on a foothold 鈥?clear the stall tracker (called each grounded tick).
    void onGrounded() {
        climbStallSinceMs = 0L;
    }
    ...
    void handleClimb(Character chr) {
        ...
        if (GCMovement.isNavigatingClimb(chr)) {
            climbStallSinceMs = 0L; // deliberate traversal 鈥?let the driver finish it
            lastClimbY = y;
            return;
        }
        if (climbStallSinceMs == 0L || Math.abs(y - lastClimbY) >= CLIMB_PROGRESS_EPS) {
            climbStallSinceMs = now(); // making progress (or first sample) 鈥?let the climb continue
            lastClimbY = y;
            return;
        }

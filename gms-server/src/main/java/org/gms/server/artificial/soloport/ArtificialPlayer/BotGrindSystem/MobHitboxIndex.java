...
public final class MobHitboxIndex {
    ...
    // How far this mob's body reaches from where it stands (px) 鈥?the single number the rest-spot
    ...
    private static Hitbox computeWithDepth(int mobId, int depth) {
        ...
        if (halfWidth == 0 && height == 0) {
            // No own frames 鈥?a link-only mob img carries its geometry under the linked id (same pattern

...
//   1. A ledge that BEARS a mob spawn is HOT and can never host a rest spot 鈥?mobs patrol their whole
...
//      (mobs don't climb 鈥?sitting a tier ABOVE a spawn is safe; the same gap sideways on the spawn floor
//      is not). A spawn-free ledge whose best point can't clear the map's inflation radius is HOT too
//      (adjacency) 鈥?this catches the low mini-shelf tucked under a spawn floor.
...
// close (nobody's watching, so a long invisible climb is wasted 鈥?it just skips the break instead).
//
...
public class RestSpotFinder {
    ...
    // Fallback inflation radius (px) 鈥?used only when a map yields no mob ids at all (WZ nor live). Real
    ...
    // breathing room on tiny-mob maps. Calibrated so big-golem maps land >= the old hand-tuned 240px 鈥?the
    ...
    // threat 鈥?being below/level gets no vertical credit.
    ...
    // grinding (only happens unobserved 鈥?an observed bot always gets a safe spot, trekking if it must).
    ...
    private static boolean isLedgeHot(ThreatContext ctx, int regionId) {
        ...
        if (l == null) {
            return false; // a rope region, not a ledge 鈥?treat as open air
        }
        return clearance(new Point(l.centerX(), l.centerY()), ctx.threats()) < ctx.inflation();
    }

    // True when a hostile mob currently sits within the inflation radius of the spot 鈥?a last-moment veto so
    // a chair never lands on a mob that wandered in after selection. Observed maps only (unobserved mobs are
    // static/irrelevant, and packets 鈥?what actually shows 鈥?are gated on observation).
    ...
    private static Point bestSafe(Character chr, int maxClimbPx, double travelWeight, boolean observed) {
        ...
        return ranked.get(0).spot(); // every safe spot momentarily occupied 鈥?take the best anyway
    }

    // GM debug (!bot restspot): EVERY reachable ledge in the observed budget with its axis-aware clearance,
    // hot classification (+ reason), the inflation radius in force, and the edge-clamped sit point 鈥?so the
    ...
    private static SafePoint safestPoint(MapleMap map, GCMovement.Ledge ledge, List<Threat> threats) {
        ...
        if (lo > hi) {
            lo = hi = ledge.centerX(); // ledge shorter than the margins 鈥?sit dead center
        }
    ...
    // The safest reachable portal's ground 鈥?the reliable "somewhere safe" answer when no ledge clears the
    // inflation floor. Each portal's ground point is run through the same threat model (a portal is NOT
    // assumed spawn-free 鈥?some maps spawn right on the entrance); pick the highest-clearance one, nearest as
    ...
    private static int inflationRadius(MapleMap map) {
        return INFLATION_CACHE.computeIfAbsent(map.getId(), id -> {
            ...
            if (maxReach == 0) {
                return DEFAULT_INFLATION_PX; // no ids anywhere 鈥?keep the hand-tuned constant
            }

...
/*
 * Immutable analytic movement plan 鈥?the shared "where / how long" contract for the LOD layer.
 *
 ...
 * pure function of elapsed wall-clock time 鈥?no runtime physics integration. This is the
 ...
 *
 * Pure data + pure math 鈥?no mutable cursor, no side effects 鈥?so it is trivially unit-testable
 * with a fake clock.
 */
final class MovementPlan {
     ...
     * (no edges) 鈥?the caller treats that as "already there".
     */
     ...
     * whichever edge is in flight. Computed on demand only (lazy) 鈥?a coarse bot pays nothing per tick

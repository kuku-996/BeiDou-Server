...
import org.gms.server.artificial.soloport.ArtificialPlayer.BotGrindSystem.MapGrindProfile;
import org.gms.server.artificial.soloport.ArtificialPlayer.BotGrindSystem.Spot;
import org.gms.server.artificial.soloport.ArtificialPlayer.BotGrindSystem.SpotFinder;
import org.gms.server.artificial.soloport.ArtificialPlayer.BotChatterSystem.BotChatter;
import org.gms.server.artificial.soloport.ArtificialPlayer.BotChatterSystem.TownChatterLines;
...
import org.gms.server.artificial.soloport.ArtificialPlayer.BotSpotClaims;
import org.gms.server.artificial.soloport.ArtificialPlayer.BotTownSystem.TownPinsStore;
...
import org.gms.server.artificial.soloport.ArtificialPlayer.GCMoveSystem.LodCounts;
import org.gms.server.artificial.soloport.Casino.CasinoChipConfig;
import org.gms.server.artificial.soloport.server.BotPerfStats;
...
import static org.gms.server.artificial.soloport.Environment.PlatformPlacement.botMoveToPlatformAnyUnoccupiedSpot;
import static org.gms.server.artificial.soloport.Environment.EnvironmentManager.environmentLoadStartup;
import static org.gms.server.artificial.soloport.Environment.PlatformPlacement.getAllCharsOnPlatform;
import static org.gms.server.artificial.soloport.Environment.PlatformPlacement.getAvailablePlatformIds;
...
import static org.gms.server.artificial.soloport.Environment.EnvironmentManager.spawnBotsInFMEntrance;
import static org.gms.server.artificial.soloport.Environment.EnvironmentManager.spawnCasinoNpcs;
import static org.gms.server.artificial.soloport.Environment.PlatformPlacement.spawnFillerBots;
import static org.gms.server.artificial.soloport.Environment.EnvironmentManager.spawnScatteredTrainingBots;
import static org.gms.server.artificial.soloport.Environment.EnvironmentManager.spawnTownPresence;
import static org.gms.server.artificial.soloport.Environment.EnvironmentManager.spawnSocialCohort;
import static org.gms.server.artificial.soloport.Environment.EnvironmentManager.spawnTownWanderers;
import static org.gms.server.artificial.soloport.Environment.EnvironmentManager.spawnFillerBotsHenesys;
import static org.gms.server.artificial.soloport.Environment.EnvironmentManager.spawnFillerBotsHenesysMarket;
import static org.gms.server.artificial.soloport.Environment.EnvironmentManager.spawnFillerBotsHenesysPark;
import static org.gms.server.artificial.soloport.Environment.EnvironmentManager.spawnFillerBotsGameZone;
import static org.gms.server.artificial.soloport.Environment.PlatformPlacement.spawnFillerBotsLockedY;
import static org.gms.server.artificial.soloport.Environment.EnvironmentManager.spawnFillerBotsPotionShop;
import static org.gms.server.artificial.soloport.Environment.EnvironmentManager.convertRandomFillersToScrollBots;
import static org.gms.server.artificial.soloport.Environment.EnvironmentManager.spawnOPQBotsInLobby;
import static org.gms.server.artificial.soloport.Environment.EnvironmentManager.spawnGameZoneHostBots;
import static org.gms.server.artificial.soloport.Environment.EnvironmentManager.spawnAttackTestBots;
import static org.gms.server.artificial.soloport.Environment.EnvironmentManager.spawnBlackjackTables;
import static org.gms.server.artificial.soloport.Environment.EnvironmentManager.spawnGachaBotsHenesys;
import static org.gms.server.artificial.soloport.Environment.EnvironmentManager.spawnHenesysBots;
import static org.gms.server.artificial.soloport.Environment.EnvironmentManager.spawnMerchBotsInFMEntrance;
...
public class EnvironmentCommand extends Command {
    ...
    // !env townpresence (alias tp) 鈥?live tuning of the ambient town-presence system (Phase 2 curation
    ...
    // !env chatter 鈥?inspect / tune the ambient bot-to-bot conversation system (BotChatterSystem).
    ...
    private static void handleChatter(String[] params, Client c) {
        ...
        switch (action) {
            ...
            case "demo" -> {
                MapleMap map = p.getMap();
                Point center = p.getPosition();
                List<Integer> ids = new ArrayList<>(2);
                for (int dx : new int[]{-45, 45}) {
                    try {
                        int id = BotGeneration.createBot(
                                new Point(center.x + dx, center.y), map, 1, 20, 20);
                        if (id > 0) {
                            ids.add(id);
                        }
                    } catch (Exception e) {
                        p.dropMessage(6, "chatter demo: bot create failed: " + e.getMessage());
                    }
                }
                BotTypeManager.setAndStartBots(ids, BotTypeManager.BotType.SOCIAL_BOT);
                int started = 0;
                for (int id : ids) {
                    BotSM bot = CharacterStorage.getBotById(id);
                    if (bot != null && BotChatter.forceChatter(bot)) {
                        started++;
                        break;
                    }
                }
                p.dropMessage(6, "chatter demo: spawned " + ids.size() + " adjacent social bot(s), started "
                        + started + " exchange(s).");
            }
            default -> {
                ...
                p.dropMessage(6, "chatter: reload | fire (force chats on this map) | demo (spawn an adjacent test pair)");
            }
        }
    }
    ...
    private static void printHelp() {
        ...
        player.yellowMessage("!env chatter demo                - spawn an adjacent SocialBot pair and start one test exchange");
    ...
    // Used to calibrate DENSITY_HI/LO / GAP_LO / SPOT_RADIUS against real maps (搂13.10 #1). Building it
    ...
    private static void dumpGrindProfile(Client c) {
        ...
        player.yellowMessage(String.format("mode: %s  (best sameLedge spawns %d 鈥?roam when below the campable floor)",
                p.roam() ? "ROAM (no campable ledge)" : "CAMP", bestSameLedge));
        if (!p.stacks().isEmpty()) {
            int bestFeed = 0;
            int hop = 0;
            for (org.gms.server.artificial.soloport.ArtificialPlayer.BotGrindSystem.SpotStack st : p.stacks()) {
                bestFeed = Math.max(bestFeed, st.totalFeed());
                if (st.hopTraversable()) {
                    hop++;
                }
            }
            player.yellowMessage(String.format("stacks: %d (best feed %d, hop-traversable %d) 鈥?STACK archetype candidates",
                    p.stacks().size(), bestFeed, hop));
        }
        player.yellowMessage(String.format("occupancy: %d bots targeting this map / capacity %d",
                org.gms.server.artificial.soloport.ArtificialPlayer.BotGrindSystem.TrainingMapChooser.botsTargeting(p.mapId()),
                org.gms.server.artificial.soloport.ArtificialPlayer.BotGrindSystem.TrainingMapChooser.mapCapacity(p.mapId())));
        if (p.spots().isEmpty()) {
            player.yellowMessage("  (no spots 鈥?nav graph unbaked or map has no spawn points)");
            return;
        }
        int totalHolders = 0;

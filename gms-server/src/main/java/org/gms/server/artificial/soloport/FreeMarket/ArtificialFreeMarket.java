package org.gms.server.artificial.soloport.FreeMarket;
...
import static org.gms.server.artificial.soloport.ArtificialPlayer.BotCustomization.getRandomChairId;
import static org.gms.server.artificial.soloport.ArtificialPlayer.BotCustomization.getRandomStorePermitId;
...
import static org.gms.server.artificial.soloport.ArtificialPlayer.BotGeneration.createBotPollReadiness;
import static org.gms.server.artificial.soloport.server.ExecutorServiceManager.runAsync;
import static org.gms.server.artificial.soloport.ArtificialPlayer.BotMovementSystem.MovementCommands.botSitChair;
import static org.gms.server.artificial.soloport.ArtificialPlayer.BotMovementSystem.MovementCommands.microTurnAround;
import static org.gms.server.artificial.soloport.DebugUtilities.debugprint;
import static org.gms.server.artificial.soloport.FreeMarket.ArtificialShopGenerator.applyAdditionalShopMods;
import static org.gms.server.artificial.soloport.FreeMarket.ArtificialShopGenerator.applyCheapSaleDiscount;
import static org.gms.server.artificial.soloport.FreeMarket.ArtificialShopGenerator.applyQuittingSaleDiscount;
import static org.gms.server.artificial.soloport.FreeMarket.ArtificialShopGenerator.buyoutSomeItems;
import static org.gms.server.artificial.soloport.FreeMarket.ArtificialShopGenerator.setOneMesoShop;
import static org.gms.server.artificial.soloport.FreeMarket.ArtificialShopGenerator.generateShop;
import static org.gms.server.artificial.soloport.FreeMarket.ArtificialShopGenerator.generateSecondaryShop;

import static org.gms.server.artificial.soloport.FreeMarket.FMEconomyManager.getTierForRoom;
import static org.gms.server.artificial.soloport.FreeMarket.FMEconomyManager.isHotRoom;
import static org.gms.server.artificial.soloport.FreeMarket.FMShopTypeManager.selectHotRoomShopType;
import static org.gms.server.artificial.soloport.FreeMarket.FMShopDescriptionManager.appendRoomNumber;
import static org.gms.server.artificial.soloport.FreeMarket.FMShopDescGen.getRandomShopOwnerIGN;
import static org.gms.server.artificial.soloport.FreeMarket.FMShopDescriptionManager.setMerchantDescription;
import static org.gms.server.artificial.soloport.FreeMarket.FMShopDescriptionManager.appendMerchantDescription;
import static org.gms.server.artificial.soloport.FreeMarket.FMShopInfoManager.getRegionByMapId;
import static org.gms.server.artificial.soloport.FreeMarket.FMShopTypeManager.getSecondaryShopItemTypes;
import static org.gms.server.artificial.soloport.FreeMarket.FMShopTypeManager.selectShopTypeWeightedProbability;

import static org.gms.server.artificial.soloport.FreeMarket.HiredMerchantArtificial.shopTypes.*;
import static org.gms.server.artificial.soloport.server.ExecutorServiceManager.getExecutorService;
import static org.gms.server.artificial.soloport.server.ExecutorServiceManager.getScheduledExecutorService;
import static org.gms.server.artificial.soloport.server.SoloMaplingUtilities.chance;
...
public class ArtificialFreeMarket {
    ...
    private static volatile boolean automaticShopsEnabled = true;
    private static final Set<Integer> automaticPlayerShopBotIds = ConcurrentHashMap.newKeySet();
    private static final AtomicInteger pendingAutomaticShops = new AtomicInteger();

    public static void setAutomaticShopsEnabled(boolean enabled) {
        automaticShopsEnabled = enabled;
    }

    public static boolean isAutomaticShopsEnabled() {
        return automaticShopsEnabled;
    }

    /**
     * Keeps a deliberately small, v83-safe number of visible automatic shops.
     * These hired merchants are separate map objects and therefore do not add
     * to the live bot population target managed by the web console.
     */
    public static void ensureAutomaticShopCount(int requestedCount) {
        if (!automaticShopsEnabled) return;

        int target = Math.max(0, Math.min(4, requestedCount));
        int needed = target - countAutomaticShops() - pendingAutomaticShops.get();
        if (needed <= 0) return;

        List<Integer> rooms = fmInfo.getRegionFMMapId("henesys");
        List<Point> positions = fmInfo.getRegionFMSpots("henesys");
        if (rooms.isEmpty() || positions.isEmpty()) return;

        for (int i = 0; i < needed; i++) {
            int mapId = rooms.get(i % rooms.size());
            Point position = positions.get(i % positions.size());
            pendingAutomaticShops.incrementAndGet();
            ExecutorServiceManager.runAsync(() -> {
                try {
                    if (automaticShopsEnabled) spawnHiredMerchantStore(mapId, position);
                } catch (Exception e) {
                    debugprint("Automatic shop creation failed: " + e.getMessage());
                } finally {
                    pendingAutomaticShops.decrementAndGet();
                }
            });
        }
    }
...
//        c.getChannelServer().closeAllMerchants();
    ...
    public static void BotPlayerStorePermit(Character fakechar) {
        if (!automaticShopsEnabled || fakechar == null || fakechar.getMap() == null) return;
        String desc = "Test";
        ...
        // A player shop is a separate map object. Merely attaching it to the
        // bot is not enough: clients need the box-update packet before they
        // can see or click the store. Normal player flow sends this packet
        // when the owner presses "Open shop"; bots have no UI, so do it here.
        PlayerShop fakecharplayershop = fakechar.getPlayerShop();
        fakecharplayershop.setOpen(true);
        fakechar.getMap().broadcastMessage(PacketCreator.updatePlayerShopBox(fakecharplayershop));
...
//            }
            ...
            automaticPlayerShopBotIds.add(fakechar2.getId());
            ...
            getScheduledExecutorService().schedule(() -> runAsync(() -> {
                if (!automaticShopsEnabled) {
                    closeAutomaticPlayerShopBot(fakechar2);
                    return;
                }
    ...
    /** Close only SoloMapling-created shops; real-player merchants are never touched. */
    public static void closeAutomaticShops() {
        for (Integer botId : new ArrayList<>(automaticPlayerShopBotIds)) {
            Character bot = BotHelpers.getCharFromChannelStorage(botId);
            if (bot != null) closeAutomaticPlayerShopBot(bot);
            automaticPlayerShopBotIds.remove(botId);
        }
        for (var channel : Server.getInstance().getAllChannels()) {
            for (Map.Entry<Integer, HiredMerchant> entry
                    : new ArrayList<>(channel.getHiredMerchants().entrySet())) {
                if (!(entry.getValue() instanceof HiredMerchantArtificial)) continue;
                channel.removeHiredMerchant(entry.getKey());
                try {
                    entry.getValue().forceClose();
                } catch (Exception e) {
                    debugprint("Failed to close automatic hired merchant: " + entry.getKey());
                }
            }
        }
    }

    public static int countAutomaticShops() {
        int count = automaticPlayerShopBotIds.size();
        for (var channel : Server.getInstance().getAllChannels()) {
            count += (int) channel.getHiredMerchants().values().stream()
                    .filter(HiredMerchantArtificial.class::isInstance)
                    .count();
        }
        return count;
    }

    private static void closeAutomaticPlayerShopBot(Character bot) {
        if (bot == null) return;
        try {
            if (bot.getPlayerShop() != null) bot.closePlayerShop();
            if (bot.getMap() != null) BotGeneration.removeBotFromServer(bot);
        } catch (Exception e) {
            debugprint("Failed to close automatic player shop bot: " + bot.getId());
        } finally {
            automaticPlayerShopBotIds.remove(bot.getId());
        }
    }
}

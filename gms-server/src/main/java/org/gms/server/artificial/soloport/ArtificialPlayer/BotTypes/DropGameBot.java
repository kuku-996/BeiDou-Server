...
public class DropGameBot extends BotSM {
    ...
    private void rejectOtherTradeRequests() {
        ...
        dprint("draining stray trade request from " + requester.getName() + " 鈥?busy");
    ...
    protected void tradeInitialized(BotTradeSM.TradeMode tradeMode) {
        ...
        if (player != null && incoming != null && incoming != player) {
            dprint("REJECT trade from " + incoming.getName()
                    + " 鈥?already engaged with " + player.getName());
        ...
        tradeDetected = true;
        player = incoming;
        dprint("tradeInitialized 鈥?partner=" + (player == null ? "?" : player.getName()));
    }
    ...
    public void updateState() {
        ...
        // Force state back to RUNNING 鈥?we manage the trade flow ourselves.
    ...
    private void partyWait() {
        ...
        // Timeout (also covers the decline case 鈥?no direct signal, player just
        // never joins).
        if (System.currentTimeMillis() > stateEndTime) {
            dprint("PARTY_WAIT: timed out waiting for player to accept");
            BotSpeak(getChr(), "Party invite timed out 鈥?mesos forfeited. Don't waste my time next round!");
    ...
    private void activeGame() {
        ...
        // Game is running 鈥?drops happen on the async scheduler.
    ...
    private void endGameCleanup() {
        // Bot is party leader 鈫?botLeaveParty disbands and auto-removes the player.
        try {

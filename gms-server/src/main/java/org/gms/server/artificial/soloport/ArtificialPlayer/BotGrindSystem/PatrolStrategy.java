...
// the NEXT ring member instead of re-scoring the whole map 鈥?a purposeful sweep back and forth across
// the field instead of either camping one spot through long lulls or teleport-shopping for the global
// argmax. Everything else 鈥?claims, bands, patience (already SPREAD-tuned via the regime knobs),
// FIGHT/WAIT, loot 鈥?is inherited from CampStrategy unchanged; only pickSpot() differs. Ours.
final class PatrolStrategy extends CampStrategy {
    ...
    // least-crowded member (sharing 鈥?the overflow story matches camp's). The just-left spot isn't
    ...
    Spot pickSpot(Character chr, MapGrindProfile p) {
        ...
        if (ring.size() < RING_MIN) {
            return super.pickSpot(chr, p); // degenerate map for a ring 鈥?behave exactly like CAMP
        }
        int n = ring.size();
        int fallbackPos = -1;

...
final class GrindLoot {
    ...
    // Let a fresh drop finish its fly-out arc and settle on the floor before the bot grabs it 鈥?instant
    // pickup looks like the item teleports into the bot mid-air. Aged against MapItem.getDropTime() (the
    // server clock stamp set when the drop spawns), so it's "this drop has been on the ground 鈮?this long".
    ...
    // the far end, vacuuming whatever the bot passes over 鈥?no hard stop, no re-approach per item. Advanced
    ...
    boolean tryWalkAndLoot(Character chr, int x0, int x1, int searchRangePx) {
        ...
        if (pos == null) {
            return true; // loot exists but we can't route this tick 鈥?still "there's loot to deal with"
        }
        ...
        // Drive-by walk to the far end 鈥?no GCMovement.stop. Re-issue only when the far target shifts past the
    ...
    private MapItem nearestCollectableDrop(Character chr, int rangePx, int x0, int x1, int yLimit) {
        ...
        for (MapObject mo : chr.getMap().getMapObjectsInRange(pos, searchSq, List.of(MapObjectType.ITEM))) {
            ...
            if (now() - mi.getDropTime() < LOOT_SETTLE_MS) {
                continue; // too fresh 鈥?let it land first; it'll be collected on a later tick
            }
    ...
    // Every eligible drop in [x0, x1] within rangePx and the same-ledge Y band 鈥?the multi-drop counterpart to
    ...
    private List<MapItem> collectableDrops(Character chr, int rangePx, int x0, int x1, int yLimit) {
        ...
        for (MapObject mo : chr.getMap().getMapObjectsInRange(pos, searchSq, List.of(MapObjectType.ITEM))) {
            ...
            if (now() - mi.getDropTime() < LOOT_SETTLE_MS) {
                continue; // too fresh 鈥?let it land first; the pass-over grabs whatever has settled by then
            }
            Point ip = mi.getPosition();
            if (ip.x < x0 || ip.x > x1 || Math.abs(ip.y - pos.y) > yLimit) {
                continue; // outside the leash, or on a stacked ledge above/below (unreachable)
    ...
    // sitting at its feet this tick 鈥?WITHOUT halting or cancelling the walk (the GCMovement.stop was what made
    ...
    // Throttled loot narration 鈥?pickups are frequent, so cap one line per LOOT_NARRATE_GAP_MS. Its

...
import org.gms.server.artificial.soloport.server.SoloMaplingConstants;
...
import static org.gms.server.artificial.soloport.ArtificialPlayer.BotDecoratorSystem.BotDecorate.setBotVariables;
import static org.gms.server.artificial.soloport.ArtificialPlayer.BotMovementSystem.MovementCommands.microTurnAroundToLeft;
import static org.gms.server.artificial.soloport.DebugUtilities.debugprint;
import static org.gms.server.artificial.soloport.FreeMarket.FMShopDescGen.getRandomCharacterIGN;
import static org.gms.server.artificial.soloport.server.ExecutorServiceManager.runAsync;
import static org.gms.server.artificial.soloport.server.SoloMaplingUtilities.getChr;
import static org.gms.server.artificial.soloport.server.SoloMaplingUtilities.channel;
import static org.gms.server.artificial.soloport.server.SoloMaplingUtilities.getMapleMapById;
import static org.gms.server.artificial.soloport.server.SoloMaplingUtilities.world;
...
public class BotGeneration {
    ...
    public static int createBot(Point pos, MapleMap map, int baseClass, int minLevel, int maxLevel, int forcedJobId) {
        ...
        // A bot is cloned from the database base character.  The base character may have
        // been saved while dead or with stale local stats; leaving those values in place
        // makes the movement renderer choose the dead stance as soon as combat starts.
        // Decorators above can also change job/level, so recalculate first and only then
        // restore the newly calculated maximum vitality.
        normalizeBotVitals(bot);
     ...
     * caller waits for the full spawn choreography to finish before continuing 鈥?     * use this (not createBot's async path) when downstream actions must not race
    ...
    /**
     * Bots are in-memory actors and must never inherit a damaged/dead state from
     * character id 2, the database template used by {@link #createBot}.
     */
    private static void normalizeBotVitals(Character bot) {
        if (bot == null) {
            return;
        }
        ...
        int maxMp = Math.max(0, bot.getCurrentMaxMp());
        if (bot.getHp() < maxHp) {
            bot.addHP(maxHp - bot.getHp());
        }
        if (bot.getMp() < maxMp) {
            bot.addMP(maxMp - bot.getMp());
        }
    }
...
//        final Channel channel = org.gms.server.getInstance().getChannel(BotSM.GameConstants.WORLD_SCANIA, BotSM.GameConstants.CHANNEL_1);
        channel.addPlayer(fakechar);
//        World world = org.gms.server.getInstance().getWorld(BotSM.GameConstants.WORLD_SCANIA);

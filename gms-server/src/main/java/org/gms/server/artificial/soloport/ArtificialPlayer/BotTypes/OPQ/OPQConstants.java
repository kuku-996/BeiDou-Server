...
public final class OPQConstants {
    ...
    // instance 鈥?never use those for filtering. Filter by data id instead.
    ...
    // 7 breakable box reactors on map 920010400 (one per platform m3鈥搈9).
    ...
    // lives at very negative y values 鈥?the sign on Y here is correct.
    ...
    // Stage 1 drop / return zone 鈥?where the leader stands near the NPC.

    // Stage 2: boxes are on platforms m3鈥搈9 (m1/m2 are base floor and entry).
    public static final List<String> STAGE_2_BOX_PLATFORMS = List.of(
            "m3", "m4", "m5", "m6", "m7", "m8", "m9"
    );

    // Stage 2 drop zone 鈥?bots navigate to the music box reactor position.

...
//
// Extension point: PatrolStrategy subclasses this and overrides pickSpot() only 鈥?the whole
...
class CampStrategy implements GrindStrategy {
    ...
    // 鈹€鈹€ Regime-adjusted patience: COMPACT maps camp through the respawn lull (dense, fast refill);
    //    SPREAD/SPARSE maps leave a dry spot sooner. Knob DEFAULTS only 鈥?the FSM is identical. 鈹€鈹€
    ...
    // 鈹€鈹€ Anchor-proximity watchdog (flash-jump classes fling themselves onto a stacked mini-platform and
    //    loop trying to dash back; the progress heartbeat doesn't catch it because they may still be
    //    landing hits down there). Escalation ladder: soft walk-home -> hard teleport-home -> relocate. 鈹€鈹€
    ...
    // 鈹€鈹€ Intra-spot spacing (sharers hold personal bands of a wide spot) 鈹€鈹€
    ...
    // Camp hops land on the spot's own ledge 鈥?the same authority the acquisition gate uses.
    @Override
    ...
    private void doSelectSpot(Character chr) {
        ...
        if (s == null) {
            return; // no reachable spot (rare 鈥?DECIDE guarantees mobs), or released mid-flight
        }
        ...
        if (within(chr, s.anchor(), s.radius())) {
            ...
            // The initial spot is sometimes on another platform.  Teleporting
            // to its grounded anchor is safer than letting combat navigation
            // pick a rope/ladder route before the first target is selected.
            if (GCMovement.onDifferentLedge(chr.getMap(), chr.getPosition().x, chr.getPosition().y,
                    s.anchor().x, s.anchor().y)) {
                GCMovement.teleportTo(chr, s.anchor().x, s.anchor().y);
            } else {
    ...
    private Spot selectAndClaim(Character chr, MapGrindProfile p) {
        synchronized (b.claimLock) {
            if (!b.claimActive) {
                return null; // release() landed while this tick was in flight (bot leaving) 鈥?claiming now would leak
            }
            // Release any spot we were holding (teleport re-select path) before picking a new one 鈥?keeps claims
            ...
            // Never got an open slot 鈫?the map is saturated (more bots than spots). Grind the best candidate
            // shared rather than idling; s/idx hold the last (best) spot scored 鈥?which, under the
            ...
            if (!claimed) {
                ...
                // shareCap, so personalBand treats it as unbanded (whole-spot leash) 鈥?correct here.
    ...
    private void doFight(Character chr) {
        ...
        // A training bot stays on its assigned platform.  Do not chase a mob
        // that wandered above/below the anchor, because the generic route may
        // start a rope climb mid-combat.
        Point botPos = chr.getPosition();
        Point mobPos = t.getPosition();
        if (botPos != null && mobPos != null
                && GCMovement.onDifferentLedge(chr.getMap(), botPos.x, botPos.y, mobPos.x, mobPos.y)) {
            b.targetOid = -1;
            enterWait(chr, s);
            return;
        }
        ...
        if (b.inAttackRange(chr, t)) {
            if (b.engage.preSwingAdjust(chr, t)) {
                return; // turned / kited / re-centred this beat 鈥?swing next beat
            }
        ...
        if (offAnchorLedge(chr, s)) {
            ...
            // mob out of range 鈥?don't chase it further off, pathfind back to the anchor (never a flash jump,
    ...
    private void doWait(Character chr) {
        ...
        // Pile cleared and no mob 鈥?hold position and wait out the respawn lull (regime-scaled: a sparse
    ...
    // 鈹€鈹€ Bands + targeting 鈹€鈹€

    // The bot's personal X band on a shared spot: claim slot s of shareCap partitions anchor卤radius via
    // BotSpotClaims.section (stable per slot for the holder's whole tenure). Null = no banding applies 鈥?    // single-capacity spot, alone on the spot, or holding a squatter slot past the cap 鈥?and the caller
    ...
    // the whole spot 鈥?kills are allowed anywhere in the spot, starvation isn't 鈥?until the band feeds
    ...
    private Monster acquireTarget(Character chr, Spot s) {
        ...
        // radius (same ledge) still gets seen and grabbed 鈥?a real player would obviously hit it.
        ...
        return null; // brief band lull 鈥?hold the segment rather than instantly poach the neighbour's
    }
    ...
    private Monster stickyTarget(Character chr, Spot s) {
        ...
        if (GCMovement.onDifferentLedge(chr.getMap(), s.anchor().x, s.anchor().y, mp.x, mp.y)) {
            return null; // moved onto a separate platform 鈥?drop it and re-acquire on our own ledge
        }
        return m;
    }
    ...
    private void approachLeashed(Character chr, Spot s, Monster mob) {
        ...
        if (now() < b.attackWalkLockUntil) {
            return; // just swung 鈥?a walking class can't move mid-attack; hold, then approach the next mob
        }
    ...
    // The BOT itself (not a mob) has ended up on a different ledge than its anchor 鈥?a mistimed flash
    ...
    // bot AGAIN soon after 鈥?that spot is a flash-jump trap. Returns true when it drove movement this tick.
    private boolean enforceAnchorProximity(Character chr, Spot s) {
        ...
        // mid-return 鈥?this positional ladder owns the off-anchor failure mode.
        b.markProgress();
        if (hardReturnAt == 0L) {
            hardReturnAt = t;
        }
        ...
        // Do not use a rope as a combat-recovery route.  A direct grounded
        // reset is preferable to repeated climb/dismount loops.
        GCMovement.teleportTo(chr, s.anchor().x, s.anchor().y);
        b.lastMoveTargetX = s.anchor().x;
    ...
    // Index by identity 鈥?pickBest returns an element OF the cached list, so this matches the spotId

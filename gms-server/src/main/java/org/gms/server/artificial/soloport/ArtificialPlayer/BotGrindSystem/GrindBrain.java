...
// (CAMP / ROAM today; PATROL / STACK planned). TrainingBot's macro brain talks only to this class 鈥?// the public surface is identical to the pre-split GrindBrain, so the macro FSM and watchdog didn't
...
public final class GrindBrain {
    ...
    // 鈹€鈹€ Shared approach/engage tunables (read by the strategies + beats) 鈹€鈹€
    ...
    // TWO thread families 鈥?the shared combat ticker runs the states, the macro tick (virtual thread)
    // runs start/release 鈥?and an unsynchronized release() racing an in-flight select could land its
    ...
    // 鈹€鈹€ Components (shared beats) + strategies (one instance each; active one switches per episode) 鈹€鈹€
    ...
    // 鈹€鈹€ Shared combat state (package-visible: strategies + beats read/write directly) 鈹€鈹€
    int targetOid = -1;
    ...
    // 鈹€鈹€ Combat heartbeat (read by TrainingBot's macro watchdog -> volatile) 鈹€鈹€
    private volatile long lastCombatProgressMs = 0L;
    private boolean wasObserved = false;

    // 鈹€鈹€ Debug narration (edge-triggered: speak only when the message changes) 鈹€鈹€
    ...
    // 鈹€鈹€ Public surface (unchanged from the pre-split GrindBrain) 鈹€鈹€
    ...
    public void tick(Character chr) {
        ...
        if (mapChanged || styleSwapRequested) {
            ...
            synchronized (claimLock) {
                if (!claimActive) {
                    return; // bot is being released 鈥?don't restart an episode under it
                }
        ...
        if (GCMovement.isClimbing(chr)) {
            // Keep the anti-stall recovery for an idle / accidental rope grab,
            // but do not cancel a committed navigation edge.  STACK episodes
            // intentionally use graph-verified ropes and ladders to change
            // floors; cancelling every climb here made all combat look like it
            // was permanently locked to one platform.
    ...
    // True while the bot is mid-rope or just dismounted 鈥?the watchdog defers escalation during recovery.
    ...
    // 鈹€鈹€ Strategy resolution + GM override 鈹€鈹€
    ...
    // 鈹€鈹€ Shared services (package-visible for the strategies + beats) 鈹€鈹€
    ...
    // The shared sticky target resolved to a live hostile Monster, with no leash validation 鈥?for
    ...
    // 鈹€鈹€ Approach-progress tracking (no-progress give-up on an unreachable target/spot) 鈹€鈹€

...
record BotMovementProfile(int totalSpeedStat, int totalJumpStat, boolean snowShoes)
    ...
    static final int HASTE_MAX_SPEED = 155;                    // our effective speed cap (鈮?real 140% held-down feel)
    ...
    static BotMovementProfile fromCharacter(Character character) {
        ...
        // jump). Pure roster logic 鈥?the thief never travels to cast it. max() so a young thief still gets 155.
    ...
    // Haste-thief lineage: Assassin鈫扝ermit鈫扤ight Lord and Bandit鈫扖hief Bandit鈫扴hadower all learn Haste at
    ...
    // True if the bot shares an active party with an online, level-PARTY_HASTE_THIEF_LEVEL+ haste-thief 鈥?    // the bot is treated as receiving that thief's party Haste. Roster-only; no map/proximity requirement.
    ...
    // higher-level bots also leap higher; 鈮? stays 100 so low-level bots are unchanged. Multiples of
     ...
     *  slipperiness client-side 鈥?the wearer gets normal walk physics on snow/ice maps. */
    ...
    private static int bucketStat(int stat) {
        ...
        // Nearest bucket, not floor: a 144% bot plays on the 145 graph 鈥?halves the

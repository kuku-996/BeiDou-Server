package org.gms.server.artificial.soloport.ArtificialPlayer.GCMoveSystem;
...
// Extracted near-verbatim from GreenCatMS (org.gms.server.bots.BotPhysicsEngine), repackaged to GCMoveSystem
...
final class BotPhysicsEngine {
    ...
    static class Config {
        ...
        public float JUMP_ROPE_PXS = 375.0f;        // rope-jump finding (NOT applied): real client kick = (卤162, -277)
        // Flash Jump (Hermit/NL) apex impulse (GreenCatMS RE'd client constants 鈥?WZ has only mpCon, no
        ...
        // Airborne horizontal control 鈥?CONFIRMED in disassembly (Angel.idb,
        ...
        //     air 鈥?the observed <= 125 px/s is the ground cap carried in by the
        //     launch snap.
        ...
        // px/s虏 so they're tick-rate independent. Ground-truth sources:
        ...
        // (`tools/swim_fit.py`) 鈥?not hand-tuned. Decodes all swim packet
        // logs (burst, burst-upheld, upheld, downheld) into 34 (vy0, dur,
        // vy_end) tuples and fits a linear-drag model dv/dt = g_eff - k路v
        ...
        public int SWIM_LEVEL_BAND_PX = 30;          // |dy| <= this = "same level" 鈫?UP hold
    ...
    // floor 鈥?the old flat 1500ms (~1000px at terminal velocity) cut long single-falls short, so no
    ...
    // Flash-jump apex impulse per tick (px/tick = px/s * tickS). Horizontal is the 卤550 client cap; vertical
    ...
    // (velY >= 0 鈥?the original GreenCatMS behavior); a reduced scale fires part-way up the ascent, so
    // the dash starts lower and lands sooner 鈥?the short flash jump for small platforms.
     ...
     * before walking off a ledge. O(1) derivation from config constants 鈥?used by graphgen to
     ...
     * Time to ~95% terminal 鈮?3/(friction+slope). We use a fixed multiple of walkStep that comfortably
     ...
     * to top speed is v_max&sup2;/(2*accel*fs) 鈥?~28 px at El Nath fs=0.2 鈥?added on top of the
     ...
     * If no foothold is found the bot has walked off the edge 鈥?physics starts a fall and this
     ...
     * ground like a real player loading in 鈥?instead of snapping straight to the floor. The normal
    ...
    static void beginGroundJump(BotMovementState entry, Character bot, int airVelX) {
        ...
        // In swim maps, physics owns horizontal motion 鈥?drop any committed
    ...
    /* Called by navigation when a DROP edge is executed 鈥?bot intentionally walks off a ledge. */
    ...
    private static void landOnGround(BotMovementState entry,
        ...
        // the air-steer direction 鈥?the bot visibly faced backwards on landing even when continuing.
  ...
  /*
    ...
    static PostLandingJump simulatePostLandingGroundTicks(MapleMap map,
        ...
        if (mapGroundSlipScale(map, profile) < 1.0) {
            // Slippery landing: stop-policy parity with the live tick (slipperyStopDir) 鈥?            // glide when the glide-out stays on ground, counter-strafe brake when it would
            ...
            for (int i = 0; i < POST_LANDING_BRAKE_TICK_CAP; i++) {
                ...
                if (dir == 0) {
                    ...
                    // ground 鈥?either way this landing is stable.
     ...
     *   swimMoveDir       鈥?-1/0/+1 horizontal steer
     *   swimVerticalHold  鈥?-1 (UP slow sink) / 0 (free sink) / +1 (DOWN fast sink)
     *   swimJumpRequested 鈥?one-shot upward burst (consumed here)
     *
     * Movement layer never writes velY/hspeed directly 鈥?the engine owns physics.
    ...
    static void applySwimMotion(BotMovementState entry) {
        ...
        // the launch (airVelX, fixedAirArc) 鈥?swim physics owns motion now.
        if (!entry.swimming) {
            entry.physX = pos.x;
            entry.physY = pos.y;
            entry.airVelX = 0;
            ...
            entry.downJumpPending = false;
            entry.downJumpGracePeriodMS = 0L;
            // Preserve velY/hspeed from launch 鈥?a jump-off-platform should
            ...
            // can advance through a foothold without registering the floor 鈥?            // bot tunnels straight through the platform. Resync to the truth.
            entry.physX = pos.x;
            entry.physY = pos.y;
        }
        ...
        if (entry.swimJumpRequested) {
            ...
            //   swim_jump = stat[+0x6c] 脳 physcfg[+0x48] 脳 speedScale 脳 5.0
        ...
        // Vertical sink cap 鈥?discrete intent picks the terminal velocity.
     ...
     * direction 鈥?mid-air input can never rebuild walk speed.
     */
    private static void applyAirSteering(BotMovementState entry, MapleMap map, int steerDir) {
        ...
        // else: ApplyForce no-op 鈥?neither accelerates nor clamps an over-band velocity
     ...
     * held for the whole flight 鈥?held input suppresses drag and is a no-op above the input
  ...
  /*
     ...
     * trajectories (fixedAirArc, JUMP/DROP edges) 鈥?a no-op above the input band that
     * keeps vx constant 鈥?and leaves moveDir=0 for free no-input flight, which gets
     ...
     * All collision outcome methods are private 鈥?movement must not call them directly.
     */
    static AirborneStepResult stepAirborne(BotMovementState entry, Character bot) {
        ...
        // trajectories (input no-op above the band 鈥?vx constant); free flight with no
        ...
        // apex (velY crosses to >= 0) with the full (卤550, -350 px/s) dash; a reduced flashJumpScale
        // (ours 鈥?level tier x platform fit) fires part-way up the ascent with a proportionally smaller
    ...
    private static int broadcastStance(BotMovementState entry, int baseStance) {
        // BeiDou's v83 move decoder only accepts the stock locomotion stance
        // table.  The donor's synthetic ALERT values (17/18) are not valid
        // locomotion states here and can render as a death/rope transition.
        // Attack packets already provide their own animation, so movement must
        // stay on its base stand/walk/jump state.
        return baseStance;
    }
    ...
    static int resolveStance(BotMovementState entry) {
        ...
        if (bot != null && bot.getHp() <= 0) {
            // Artificial players are simulation actors: they must not inherit a dead
            // template state (or render as dead after an external packet) while their
            // training FSM is still active. Keep their visual and server HP in sync.
            int maxHp = Math.max(1, bot.getCurrentMaxHp());
            bot.addHP(maxHp - bot.getHp());
        }
        if (entry.climbing) {
            return entry.climbRope != null && entry.climbRope.isLadder()
                    ? CharacterStance.ladderStance(entry.facingDir)
                    : CharacterStance.ropeStance(entry.facingDir);
        }
        if (entry.swimming) {
            // Training bots never need a dedicated swim state.  Use the
            // protocol-safe airborne visual until water movement is implemented
            // with captured v83 packets.
    ...
    /* forbidFallDown footholds are never pass-through 鈥?they stay solid even inside a
    ...
    // A down-jump falls through the platform and keeps falling until it lands 鈥?NO drop-distance
    ...
    // (it's neither a 300px probe nor purely the forbidFallDown flag) 鈥?until it's pinned, generate
    ...
    private static boolean resolveClimbBoundary(BotMovementState entry, Character bot, Rope rope, int candidateY) {
        if (candidateY <= rope.topY()) {
            ...
            if (landing != null) {
                ...
                // If none is found, clamp to topY and hold rather than falling 鈥?the bot will
    ...
    // Ours (SoloMapling): widened top-exit probe 鈥?see Fable Handoff 2026-07-07.
    ...
    // acceptable foothold, and hung 鈥?ClimbRecovery then kicked it off sideways and it re-climbed.
     ...
     * LINEAR velocity ramps 鈥?constant accel SLIP_WALK_ACCEL_PXSS x fs while a
     ...
     * while acceleration and braking take 1/fs as long 鈥?slow starts, long slides. (It was
     ...
     *  BotMovementProfile) cancel the field's slipperiness 鈥?normal physics on snow. */
     ...
     * SLIP_WALK_ACCEL_PXSS x fs 鈥?3.5x the passive glide 鈥?exactly like a player
     ...
     * 80*fs px/s^2 鈥?what real players look like when they let go) unless the projected
     * input-free glide-out would slide off the ground, in which case counter-strafe brake 鈥?     * a player saving themselves near an edge. Projection reuses the live ground sim so
     ...
     * instead of overshoot 鈥?callers steering into a launch WINDOW pass the room between the
     ...
     *  @ 0x9b2c3c) 鈥?the observed |vx| <= walkSpeed is the ground cap carried in by the
     ...
     * units (packet-verified: standing/no-input jumps carry the current hspeed 鈥?0 -> 0, 3 -> 3,
     * 9 -> 10, 29 -> 29 px/s 鈥?while a held direction snaps to +-walkSpeed instead).
     */
    ...
    // before the move ends 鈥?e.g. grabbing a rope whose climbable span hangs below the ledge.
     ...
     * wasn't cached yet 鈥?which is exactly the case DURING graph building, making graphgen
    ...
    // foothold lists) 鈥?callers fall back to the original tree/map query so stubbed seams keep
        ...
        return COLLISION_INDEX.computeIfAbsent(tree, t -> {
            ...
            if (ground.isEmpty()) {
                ...
                for (int i = 0; i < bucketCount; i++) {
                    ...
                    // NON-TRANSITIVE partial order 鈥?TimSort (List.sort/Collections.sort on 32+
    ...
    static Foothold findBelowIndexed(MapleMap map, Point p) {
        ...
        if (index == UNINDEXABLE) {
            return map.getFootholds().findBelow(p); // stubbed tree 鈥?original query path
        }
    ...
    static Point pointBelowIndexed(MapleMap map, Point initial) {
        ...
        if (map.getFootholds() == null || collisionIndex(map) == UNINDEXABLE) {
            return map.getPointBelow(initial); // stubbed map/tree 鈥?original query path
        }
    ...
    // Furthest walkable foothold directly ABOVE p within maxRise px 鈥?the up-teleport mirror of
    ...
    // Verbatim port of the foothold tree's slope interpolation 鈥?the trig chain reduces to
    ...
    private static double landingGroundHSpeed(MapleMap map,
        ...
        // Client landing rule (packet-verified at fs=1 AND fs=0.2 鈥?logs/monitored-packets
        ...
        // it outright 鈥?handled by the caller, which knows the input state.)
        landingDeltaX *= 0.5;
    ...
    private static RopeGrabResult simulateRopeGrabCore(MapleMap map,
        ...
        for (int tick = 0; tick < FALL_SIM_TICK_CAP; tick++) {
            ...
            if (intY > floorY) {
                return null; // fell past the map floor 鈥?no rope to grab below
            }
            previousIntY = intY;
        }
    ...
    private static JumpLanding simulateLanding(MapleMap map,
        ...
        for (int tick = 0; tick < FALL_SIM_TICK_CAP; tick++) {
            ...
            if (intY > floorY) {
                return null; // fell past the map floor 鈥?no foothold exists below
            }
            previousIntY = intY;
        }

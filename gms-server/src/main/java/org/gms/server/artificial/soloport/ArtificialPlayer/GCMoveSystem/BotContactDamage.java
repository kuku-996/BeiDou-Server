...
final class BotContactDamage {
    ...
    static void tickMobDamage(BotMovementState entry, Character bot) {
        // TrainingBot owns its combat presentation.  Do not even enter this
        // cosmetic touch-damage path while it is grinding: a v83 client can
        // interpret an artificial DAMAGE_PLAYER packet as a real death when
        // it arrives close to a skill/mob state packet.
        if (entry.grinding) {
    ...
    private static void applyMobHit(BotMovementState entry, Character bot, Monster mob) {
        // Training bots already have a combat brain that selects targets, attacks and
        // returns to its camp.  Feeding the cosmetic contact-hit physics back into that
        // loop cancels its navigation and launches a knockback arc; near ropes this can
        // be replanned as an unintended climb.  Keep the regular i-frame bookkeeping,
        // but make contact harmless and motionless while actively grinding.
        if (entry.grinding) {
    ...
    static void applyFallDamage(BotMovementState entry, Character bot, float fallDistancePx) {
        // Do not inject a second knockback state into an active training loop.  Falls
        // are still resolved by BotPhysicsEngine; this only suppresses the cosmetic
        // hurt packet/recoil that can redirect the bot to a rope edge.
        if (entry.grinding) {
            return;
        }
        if (!GCMovement.isMapObserved(bot.getMapId())) {

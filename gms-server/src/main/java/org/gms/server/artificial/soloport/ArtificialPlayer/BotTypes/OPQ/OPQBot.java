package org.gms.server.artificial.soloport.ArtificialPlayer.BotTypes.OPQ;

import org.gms.client.Character;
import org.gms.server.maps.MapleMap;
import org.gms.server.maps.MapObject;
import org.gms.server.maps.Reactor;
import org.gms.server.artificial.soloport.ArtificialPlayer.BotCommandsPack.BotAttack;
import org.gms.server.artificial.soloport.ArtificialPlayer.BotCommandsPack.DropCommands;
import org.gms.server.artificial.soloport.ArtificialPlayer.BotCommandsPack.SocialCommands;
import org.gms.server.artificial.soloport.ArtificialPlayer.BotLogic;
...
import org.gms.server.artificial.soloport.ArtificialPlayer.BotPartySystem.BotPartyLogic;
import org.gms.server.artificial.soloport.ArtificialPlayer.BotSM;
import org.gms.server.artificial.soloport.ArtificialPlayer.BotTypes.OPQ.OPQSharedContext.OPQPhase;
import org.gms.server.artificial.soloport.Environment.EnvironmentManager;
import org.gms.server.artificial.soloport.Environment.PlatformPlacement;
import org.gms.server.artificial.soloport.MapVFX.CustomReactor;
...
import static org.gms.server.artificial.soloport.ArtificialPlayer.BotClientHandler.getBotClient;
...
import static org.gms.server.artificial.soloport.ArtificialPlayer.BotHelpers.blockingSleep;
import static org.gms.server.artificial.soloport.ArtificialPlayer.BotTypes.OPQ.OPQConstants.STAGE_1_COMPLETE_TP;
import static org.gms.server.artificial.soloport.ArtificialPlayer.BotTypes.OPQ.OPQConstants.STAGE_1_ENTRY_TP;
...
public class OPQBot extends BotSM {
     ...
     * single most useful field for tracing why a bot moved 鈥?write it as a
    ...
    private void handleInPartyIdle() {
        ...
        if (!isInParty()) {
            // Disbanded before PQ started 鈥?back to lobby chat.
            transitionTo(OPQBotState.RECRUITMENT, "party disbanded before PQ started");
            return;
        }

        // Refresh leader id every tick 鈥?covers leader change / re-invite.
//        orchestrator.noteLeaderFromBot(this);
...
//        if (reactorOid == null) {
//            // No live reactor available 鈥?every cloud is taken or already broken.
//            startStageWaitTimer();
...
//        }
        ...
        if (reactor == null || !reactor.isAlive() || reactor.getState() >= 4) {
            // Stale assignment 鈥?clear and retry next tick.
            sharedContext.putCloudAssignment(getChr().getId(), null);
            debugLogf("Reactor oid=" + reactorOid + " is stale (null/dead/state>=4) 鈥?re-requesting next tick");
            return;
        }
    ...
    private void handleStage1HitReactor() {
        ...
        if (reactorOid == null) {
            // Lost our assignment somehow 鈥?go back to navigate to re-request.
            transitionTo(OPQBotState.STAGE_1_NAVIGATE, "no reactor assignment in HIT state");
            return;
        }
        Reactor reactor = getChr().getMap().getReactorByOid(reactorOid);
        if (reactor == null || !reactor.isAlive() || reactor.getState() >= 4) {
            debugLogf("Reactor oid=" + reactorOid + " already dead 鈥?skipping to LOOT");
        ...
        // tick window 鈥?overshooting state 4 crashes the map.
        if (reactor.getState() >= 4) {
            debugLogf("Pre-hit guard tripped: state=" + reactor.getState() + " 鈥?skipping to LOOT");
...
//        }
        ...
        if (!moreBoxesLeft && reactorOid == null) {
            // All boxes broken or claimed 鈥?if we have an item, go drop it first
            if (lootedRecordItemId > 0) {
                ...
                transitionTo(OPQBotState.STAGE_2_WAIT,
                        "no boxes left and nothing to drop 鈥?waiting for stage clear");
            }
            return;
        }
        ...
        if (reactor == null || !reactor.isAlive() || reactor.getState() >= 4) {
            sharedContext.putBoxAssignment(getChr().getId(), null);
            debugLogf("Box reactor oid=" + reactorOid + " is stale (null/dead/state>=4) 鈥?re-requesting next tick");
            return;
        }
    ...
    private void handleStage2HitBox() {
        ...
        Reactor reactor = getChr().getMap().getReactorByOid(reactorOid);
        if (reactor == null || !reactor.isAlive() || reactor.getState() >= 4) {
            debugLogf("Box reactor oid=" + reactorOid + " already broken 鈥?skipping to LOOT");
    ...
    private void handleStage2DropItems() {
        ...
        // Check if more boxes remain 鈥?loop back if so
    ...
    private void handlePQAbandoned() {
        debugLogf("Party lost 鈥?PQ abandoned. Cleaning up and warping out.");
        ...
        resetOPQBotState();
        transitionTo(OPQBotState.RECRUITMENT, "party lost 鈥?PQ abandoned, returned to lobby");
    }
    ...
    private void followLeaderOut() {
        ...
        if (leaderMap == OPQConstants.OPQ_EXIT_LOBBY) {
            OPQOrchestrator.getInstance().followLeaderWarp(getChr(), new Point(-161, 323));
            transitionTo(OPQBotState.EXIT_LOBBY, "leader already left stage 2 鈥?following to exit lobby");
        } else {
            OPQOrchestrator.getInstance().followLeaderWarp(getChr(), new Point(-233, 174));
            transitionTo(OPQBotState.LOOP_CHECK, "leader already in OPQ lobby 鈥?skipping exit");
        }
    }

...
class BotMovementManager {
    ...
    private static void clearTransientState(BotMovementState entry) {
        // GCMoveSystem: grindTarget / fidget are out of scope (pure movement) 鈥?dropped.
    ...
    static void tickClimbing(BotMovementState entry, Point targetPos, boolean runAiTick) {
        ...
        try {
            ...
            // Null rope is handled inside advanceClimb/holdClimb 鈥?they call beginFall internally.
            ...
            if (!runAiTick && entry.navEdge == null) {
                // No committed nav edge 鈫?no AI-decided climb intent. On non-AI ticks the
                ...
                // off the rope-top onto the foothold above 鈥?pathlog-Preston-2026-05-07 oscillation.
    ...
    static boolean shouldSnapToClimbTarget(BotMovementState entry, Point targetPos, int dy) {
        ...
        // grinding the climb integrator against a fixed-step overshoot 鈥?every step landed
    ...
    static void tickAirborne(BotMovementState entry, Point targetPos) {
        ...
        try {
            ...
            // edges) instead fly with the LAUNCH key held 鈥?like the real player performing
            ...
            // CONTINUE 鈥?position advanced, check for rope grab at new position
            if (successfullyGrabbedRope(entry, bot, bot.getPosition())) {
                return;
            }
     ...
     * No continuous velocity steering 鈥?physics integrates the intents.
     */
    private static void computeSwimIntents(BotMovementState entry, Point targetPos) {
        ...
        // dy to oscillate across the LEVEL_BAND boundary every tick 鈥?bot
        ...
        if (targetPos == null) {
            // Idle in water 鈥?hold UP so the bot doesn't sink endlessly.
        ...
        // Hold UP just to maintain altitude, no burst, no horizontal push 鈥?        // prevents the jump/sink oscillation when bot overshoots target by a
        ...
        // band (LEVEL < dy <= DOWN) is "sticky" 鈥?we keep whichever hold was
        ...
        if (dy <= -jumpTrigger && now >= entry.swimNextJumpAtMs) {
            ...
            entry.swimVerticalHold = -1;        // clearly above target 鈫?UP
        } else if (dy > downBand) {
            entry.swimVerticalHold = 1;         // clearly far below 鈫?DOWN
        } else {
            // Middle band: persist last hold to avoid stutter. If we were
            // sinking (free or DOWN), keep that 鈥?UP would just slow our
    ...
    private static MoveAction planGroundAction(BotMovementState entry, Foothold currentFh, Point botPos, Point targetPos) {
        ...
        // No hysteresis when navigating to an edge 鈥?always move toward the waypoint
        ...
        // GCMoveSystem: mob-dodge is out of scope (pure movement) 鈥?always walk.
        return MoveAction.walk(stepX);
    }
    ...
    private static int launchWindowOvershootSlackPx(BotMovementState entry, int botX, int targetX) {
        ...
        // launch point 鈥?never allow sliding deeper into a wide window than that gate accepts.
    ...
    private static int resolveAirVelocityX(BotMovementState entry, MapleMap map, BotMovementProfile profile, int dx) {
        ...
        // slow icy start, -34 -> -124 px/s at takeoff) 鈥?see Config.AIR_CONTROL_ACCEL_PXSS.
        // Graph jump edges are calibrated at 卤walkStep of their OWN profile, so this matches
        // the simulated arc as long as planning and execution share a graph 鈥?which
    ...
    private static void sendMovementPacket(Character bot, BotPhysicsEngine.MovementSnapshot snapshot, int fhId) {
        ...
        data[11] = (byte) (fhId >> 8);
        // This is the last boundary before a synthetic movement fragment enters
        // the v83 client.  Never forward donor-only stance values (notably 17/18)
        // or normal-attack values through a type-0 locomotion fragment: BeiDou's
        // decoder renders those as a rope/death-like transition.  Attack packets
        // carry their own pose, so an unsupported value must degrade to standing.
        data[12] = (byte) sanitizeV83LocomotionStance(snapshot.stance(), snapshot.velX());
    ...
    private static int sanitizeV83LocomotionStance(int stance, int velocityX) {
        return switch (stance) {
            case 2, 3, 4, 5, 6, 7, 10, 11, 12, 13, 14, 15 -> stance;
            default -> velocityX < 0 ? 5 : 4;
        };
    }
    ...
    // sendMovementPacket above can't express 鈥?PacketCreator.movePlayer copies the bytes as-is.
    ...
    // CMovePath::Decode, a lone type-6 fragment reads no x/y 鈥?it copies the previous fragment's position
    ...
    // shorts are velocity slots, not a position delta 鈥?the dash ANIM is triggered by the fragment TYPE, so
    ...
    static void broadcastFlashJump(BotMovementState entry, int relDx, int relDy) {
        ...
        data[i++] = 0;                       // cmd 0 鈥?absolute, anchors the fj fragment's position
        data[i++] = (byte) (x & 0xFF);
        data[i++] = (byte) (x >> 8);
        data[i++] = (byte) (y & 0xFF);
        data[i++] = (byte) (y >> 8);
        ...
        data[i++] = (byte) (dur >> 8);
        data[i++] = 6;                       // cmd 6 "fj" 鈥?RelativeLifeMovement (plays the dash action)
        data[i++] = (byte) (relDx & 0xFF);
        data[i++] = (byte) (relDx >> 8);
        data[i++] = (byte) (relDy & 0xFF);
        data[i++] = (byte) (relDy >> 8);
        data[i++] = (byte) stance;
        data[i++] = 0;                       // fj duration 0 鈥?matches real captures
        data[i] = 0;
        InPacket packet = new ByteBufInPacket(Unpooled.wrappedBuffer(data));
        Packet movePacket = PacketCreator.movePlayer(bot.getId(), packet, data.length);
        bot.getMap().broadcastMessage(bot, movePacket, false);

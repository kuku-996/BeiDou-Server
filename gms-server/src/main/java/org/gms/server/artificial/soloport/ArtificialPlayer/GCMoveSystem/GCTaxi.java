...
/*
 * Basic taxi connectivity: the Victoria Island town cab network. Town鈫攖own has no walkable portal
 * path, so GCTravel can't reach e.g. Henesys鈫扠erning by walking. This adds those edges (so routing
 * finds them) and the executor rides them by walking to the cab NPC, then warping to the
 * destination town 鈥?"stand near the cab, ride." No fares/economy (pure-movement scope).
 *
...
final class GCTaxi {
    ...
    // {townMapId, cabNpcId} 鈥?the fully-connected Victoria Island cab network.

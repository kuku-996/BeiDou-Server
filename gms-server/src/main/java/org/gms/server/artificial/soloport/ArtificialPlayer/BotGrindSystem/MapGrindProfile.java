...
// measurement the grind sub-FSM reads: SELECT_SPOT scores `spots` for a bot (the 搂4 score + a per-bot
// reachability filter), and WAIT's relocate patience is regime-adjusted (GrindBrain.waitPatienceMs /
// unproductiveMs 鈥?COMPACT camps through lulls, SPREAD/SPARSE leave a dry spot sooner). The regime label
// only nudges knob DEFAULTS (patience / relocate-eagerness) and debug narration 鈥?it NEVER branches the

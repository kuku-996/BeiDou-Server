...
// across the levels 鈥?each works its own floor of the tree and hunts neighbouring floors only when
...
final class StackStrategy implements GrindStrategy {
    ...
    // The tether's X range (all beats 鈥?turn, kite, hop, loot 鈥?stay inside the stack).
    @Override
    ...
    private void doSelectStack(Character chr) {
        ...
        if (p == null || p.stacks().isEmpty()) {
            ...
            // select 鈥?the macro watchdog bails the map if this persists.
    ...
    private boolean selectAndClaim(Character chr, MapGrindProfile p) {
        synchronized (b.claimLock) {
            if (!b.claimActive) {
                return false; // released mid-flight 鈥?claiming now would leak
            }
            releaseClaimLocked();
            Point pos = chr.getPosition();
    ...
    // Best traversable stack for this bot: feed-heavy, uncrowded, near 鈥?the camp pickBest shape
    ...
    // The claim key for a stack: its feed-heaviest member's spot index (stable 鈥?the profile's spot
    ...
    private Monster acquireTarget(Character chr, SpotStack st) {
        ...
        return null; // brief floor lull 鈥?hold the level rather than instantly poach a neighbour's
    }

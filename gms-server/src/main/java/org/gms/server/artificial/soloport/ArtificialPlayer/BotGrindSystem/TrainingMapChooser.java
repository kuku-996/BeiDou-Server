...
// WHICH map a training bot grinds: the whole selection cluster extracted from TrainingBot 鈥?the
...
public final class TrainingMapChooser {
    ...
    // admits everything down to lvl-1 (snails right outside town 鈥?the lifeblood of the low-level areas)
    // AND up to level+12, while a level-80's floor sits at 55 so pros stay off trivial mobs. Levels are
    // only a small piece 鈥?near/far + crowd (distanceWeight, capacity) do the fine selection in the band.
    ...
    // with the admission band) 鈥?this only bites the fallback edge, where nothing was in-band at all.
    ...
    // 鈥?which the finder's FALLBACK now also respects 鈥?and by suppressing chill visits. See DeepHub.
    private static final int DOWNWARD_HUB_LOWER_SPAN = 10;

    // 鈹€鈹€ Chill visits (intentional, rare social variety) 鈹€鈹€
    ...
    // 鈹€鈹€ Wander radius (hops from town) 鈹€鈹€
    ...
    // 鈹€鈹€ Distance bias (fresh bots hug town, high bots venture to the edge of their reach) 鈹€鈹€
    ...
    // Pick a level-appropriate, uncrowded map for this bot and RESERVE an occupancy slot on it 鈥?the
    ...
    // last slot at the same instant), release and re-pick 鈥?the bumped count now hard-zeros that map,
    ...
    public static TrainingMap choose(Character chr, int homeMapId, Set<Integer> excluded, Consumer<String> debug) {
        ...
        // itself 鈥?the map BFS drops its own origin, so admit the current map explicitly when the bot
        ...
        // NEAR-TOWN map 鈥?a strictly below-comfort band capped near town; nothing to chill at -> retry
        ...
        if (downwardOnly) {
            ...
            hardMinMob = minMob; // the finder's fallback respects this too 鈥?a pro is never offered an easy up-map
        }
        ...
        return null; // not reached 鈥?the loop always returns on its last attempt
    }

    // 鈹€鈹€ Occupancy registry 鈹€鈹€
    ...
    // estimate over the static WZ spawn positions 鈥?no live map load, no nav bake.
    public static int mapCapacity(int mapId) {
        return SpotFinder.mapBotCapacity(mapId);
    }

    // 鈹€鈹€ Selection math 鈹€鈹€
    ...
    static int hopsForLevel(int level) {
        ...
        return MAX_HOPS; // above the top breakpoint but below ANYWHERE_LEVEL 鈥?safety only
    }

    // Weighted random over eligible maps: level-appropriate maps full weight, easier maps a reduced chance,
    // biased by distance (low bots near town, high bots further out), and CAPACITY-AWARE 鈥?a map at/over
    ...
    private static TrainingMap weightedPick(List<TrainingMap> maps, int level, int reach) {
        ...
        if (total <= 0) {
            // Every reachable map is at capacity 鈥?re-weight with the soft divisor so the surplus still

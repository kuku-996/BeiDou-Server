    ...
    public boolean removePlayer(Character character) {
        for (int i = 1; i < players.size(); i++) { // never remove dealer at index 0
            if (players.get(i).getCharacter().equals(character)) {
                ...
                //  - if removed before current 鈫?shift index down
                //  - if removed at/after current 鈫?no change (what was next is now at currentPlayerIndex)
                //  - if index now past end 鈫?wrap to dealer (0)
